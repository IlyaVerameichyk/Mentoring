...
namespace IQueryable
{
	public class E3SEntitySet<T> : IQueryable<T> where T : E3SEntity
	{
		...
		public E3SEntitySet(string user, string password)
		{
			...
			var client = new Sample03.E3SClient.E3SQueryClient(user, password);

			provider = new E3SLinqProvider(client);
		}

	    public E3SEntitySet(IHttpClient _client)
	    {
	        expression = Expression.Constant(this);

	        var client = new Sample03.E3SClient.E3SQueryClient(_client);
